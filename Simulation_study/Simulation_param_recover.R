...
for (i in 1:50) {
        print(i)
        sim_i <- sim_gdmm_data(n = 50, 
                               m = 1,
                               beta_range = c(0.5,1.5), 
                               sigma_re_range = c(0.1, 0.1),
                               sd_range = c(0.05, 0.05))  
...
ggplot(coef_table, aes(x = real, y = estimate_gdmm)) + 
        geom_point() +  
        geom_linerange(aes(ymin = estimate_gdmm - 1.96*se_gdmm, ymax = estimate_gdmm + 1.96*se_gdmm )) +
        facet_wrap(~para, scales = 'free') +
        geom_abline(slope = 1, intercept = 0)

ggplot(coef_table, aes(x = real, y = estimate_lcbd)) + 
        geom_point() +  
        geom_linerange(aes(ymin = estimate_lcbd - 1.96*se_lcbd, ymax = estimate_lcbd + 1.96*se_lcbd )) +
        facet_wrap(~para, scales = 'free') +
        geom_abline(slope = 1, intercept = 0)


ggplot(coef_table, aes(x = real, y = estimate_bbgdmm)) + 
        geom_point() +  
        geom_linerange(aes(ymin = low_bbgdmm, ymax = high_bbgdm )) +
        facet_wrap(~para, scales = 'free') +
        geom_abline(slope = 1, intercept = 0)
